...
export default function SellerProductsPage() {
  ...
  const loadMeta = async () => {
    try {
      ...
      setCategories(categoryList); setBrands(list(b.data));
    ...
    try {
      ...
      if (!editingId) {
        ...
        for (const v of form.variants) {
          ...
          if (v.id) {
            await api.put(`/seller/products/${editingId}/variants/${v.id}`, data);
            existingIds.push(v.id);
          } else {
            const res = await api.post(`/seller/products/${editingId}/variants`, data);
        ...
        if (mainFiles.length) {
          ...
          await api.post(`/seller/products/${editingId}/images`, fd);
        }
        ...
        for (const v of form.variants) {
          ...
          await api.post(`/seller/products/${editingId}/variants/${v.id}/images`, fd);
        }
  ...
  const deleteProduct = async (id: number) => {
    ...
    try {
      setLoading(true); await api.delete(`/seller/products/${id}`);
  ...
  const exportProducts = async () => {
    try {
      setLoading(true);
      const res = await api.get("/seller/products/export", { responseType: "blob" });
  ...
  const importProducts = async (file: File) => {
    try {
      ...
      await api.post("/seller/products/import", fd);
